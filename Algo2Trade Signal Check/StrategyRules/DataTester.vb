...
Public Class DataTester
    ...
    Public Overrides Async Function RunAsync(ByVal startDate As Date, ByVal endDate As Date) As Task(Of DataTable)
        ...
        ret.Columns.Add("SnapshotDateTime")
        ret.Columns.Add("TradingSymbol")
        ...
        While chkDate <= endDate
            ...
            If stockList IsNot Nothing AndAlso stockList.Count > 0 Then
                For Each stock In stockList
                    _canceller.Token.ThrowIfCancellationRequested()
                    Dim xStockPayload As Dictionary(Of Date, Payload) = Nothing
                    Dim nifty50Payload As Dictionary(Of Date, Payload) = Nothing
                    Select Case _category
                        Case Common.DataBaseTable.Intraday_Cash, Common.DataBaseTable.Intraday_Commodity, Common.DataBaseTable.Intraday_Currency, Common.DataBaseTable.Intraday_Futures
                            xStockPayload = _cmn.GetRawPayload(_category, stock, chkDate.AddDays(-8), chkDate)
                            nifty50Payload = _cmn.GetRawPayload(Common.DataBaseTable.Intraday_Cash, "NIFTY 50", chkDate.AddDays(-8), chkDate)
                        Case Common.DataBaseTable.EOD_Cash, Common.DataBaseTable.EOD_Commodity, Common.DataBaseTable.EOD_Currency, Common.DataBaseTable.EOD_Futures, Common.DataBaseTable.EOD_POSITIONAL
                            xStockPayload = _cmn.GetRawPayload(_category, stock, chkDate.AddDays(-200), chkDate)
                            nifty50Payload = _cmn.GetRawPayload(Common.DataBaseTable.EOD_Cash, "NIFTY 50", chkDate.AddDays(-200), chkDate)
                        Case Common.DataBaseTable.Intraday_Futures_Options
                            xStockPayload = _cmn.GetRawPayloadForSpecificTradingSymbol(_category, stock, chkDate.AddDays(-8), chkDate)
                            nifty50Payload = _cmn.GetRawPayload(Common.DataBaseTable.Intraday_Cash, "NIFTY 50", chkDate.AddDays(-8), chkDate)
                        Case Common.DataBaseTable.EOD_Futures_Options
                            xStockPayload = _cmn.GetRawPayloadForSpecificTradingSymbol(_category, stock, chkDate.AddDays(-200), chkDate)
                            nifty50Payload = _cmn.GetRawPayload(Common.DataBaseTable.EOD_Cash, "NIFTY 50", chkDate.AddDays(-200), chkDate)
                    End Select
                    _canceller.Token.ThrowIfCancellationRequested()
                    If xStockPayload IsNot Nothing AndAlso xStockPayload.Count > 0 AndAlso
                        nifty50Payload IsNot Nothing AndAlso nifty50Payload.Count > 0 Then
                        Dim dateList As List(Of Date) = nifty50Payload.Keys.ToList
                        Dim preDate As Date = Date.MinValue
                        For Each runningDate In dateList
                            _canceller.Token.ThrowIfCancellationRequested()
                            If Not xStockPayload.ContainsKey(runningDate) Then
                                If preDate <> Date.MinValue AndAlso xStockPayload.ContainsKey(preDate) Then
                                    xStockPayload.Add(runningDate, xStockPayload(preDate))
                                End If
                            End If
                            preDate = runningDate
                        Next
                        ...
                        For Each runningPayload In xStockPayload.OrderBy(Function(x)
                                                                             Return x.Key
                                                                         End Function)
                            If stockPayload Is Nothing Then stockPayload = New Dictionary(Of Date, Payload)
                            stockPayload.Add(runningPayload.Key, runningPayload.Value)
                        Next
                        ...
                        If currentDayPayload IsNot Nothing AndAlso currentDayPayload.Count > 0 Then
                            For Each runningPayload In currentDayPayload.Keys
                                ...
                                row("SnapshotDateTime") = runningPayload.ToString("dd-MM-yyyy HH:mm:ss")
                                row("TradingSymbol") = inputPayload(runningPayload).TradingSymbol
