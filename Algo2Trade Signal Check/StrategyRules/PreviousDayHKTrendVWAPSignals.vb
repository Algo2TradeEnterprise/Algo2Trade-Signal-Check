...
Public Class PreviousDayHKTrendVWAPSignals
    Inherits Rule
                        ...
                        If currentDayPayload IsNot Nothing AndAlso currentDayPayload.Count > 0 Then
                            ...
                            Select Case _category
                                Case Common.DataBaseTable.Intraday_Cash
                                    eodPayload = _cmn.GetRawPayload(Common.DataBaseTable.EOD_Cash, stock, chkDate.AddDays(-200), chkDate.AddDays(-1))
                                Case Common.DataBaseTable.Intraday_Commodity
                                    eodPayload = _cmn.GetRawPayload(Common.DataBaseTable.EOD_Commodity, stock, chkDate.AddDays(-200), chkDate.AddDays(-1))
                                Case Common.DataBaseTable.Intraday_Currency
                                    eodPayload = _cmn.GetRawPayload(Common.DataBaseTable.EOD_Currency, stock, chkDate.AddDays(-200), chkDate.AddDays(-1))
                                Case Common.DataBaseTable.Intraday_Futures
                                    eodPayload = _cmn.GetRawPayload(Common.DataBaseTable.EOD_Futures, stock, chkDate.AddDays(-200), chkDate.AddDays(-1))
                                Case Else
                            ...
                            If eodPayload IsNot Nothing AndAlso eodPayload.Count > 30 Then
                                ...
                                Dim lastDayPayload As Payload = eodHKPayload.LastOrDefault.Value
                                Dim direction As Integer = 0
                                If Math.Round(lastDayPayload.Open, 2) = Math.Round(lastDayPayload.Low, 2) Then
                                    direction = 1
                                ElseIf Math.Round(lastDayPayload.Open, 2) = Math.Round(lastDayPayload.High, 2) Then
                                    direction = -1
                                End If
                                If direction <> 0 Then
                                    ...
                                    For Each runningPayload In currentDayPayload
                                        _canceller.Token.ThrowIfCancellationRequested()
                                        If direction = 1 AndAlso runningPayload.Value.High < vwapPayload(runningPayload.Key) AndAlso
                                            Math.Round(runningPayload.Value.Open, 2) = Math.Round(runningPayload.Value.High, 2) Then
                                            ...
                                            ret.Rows.Add(row)
                                        ElseIf direction = -1 AndAlso runningPayload.Value.Low > vwapPayload(runningPayload.Key) AndAlso
                                            Math.Round(runningPayload.Value.Open, 2) = Math.Round(runningPayload.Value.Low, 2) Then
                                            Dim row As DataRow = ret.NewRow
                                            row("Date") = runningPayload.Value.PayloadDate.ToString("dd-MMM-yyyy HH:mm:ss")
                                            row("Trading Symbol") = runningPayload.Value.TradingSymbol
                                            row("Open") = Math.Round(runningPayload.Value.Open, 2)
