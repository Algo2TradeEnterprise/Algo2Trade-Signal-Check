                    ...
                    If stockPayload IsNot Nothing AndAlso stockPayload.Count > 0 Then
                        ...
                        If currentDayPayload IsNot Nothing AndAlso currentDayPayload.Count > 0 Then
                            ...
                            For Each runningPayload In currentDayPayload.Keys
                                ...
                                '    End If
                                'End If
                                If currentDayPayload(runningPayload).Close > candleToCheck.Close Then
                                    If currentDayPayload(runningPayload).Close > leadingSpanAPayload(runningPayload) AndAlso
                                        currentDayPayload(runningPayload).Close > leadingSpanBPayload(runningPayload) Then
                                        Dim row As DataRow = ret.NewRow
                                        row("Date") = inputPayload(runningPayload).PayloadDate.ToString("dd-MMM-yyyy")
                                        row("Time") = inputPayload(runningPayload).PayloadDate.ToString("HH:mm:ss")
                                        row("Trading Symbol") = inputPayload(runningPayload).TradingSymbol
                                        ...
                                        ret.Rows.Add(row)
                                    End If
                                ElseIf currentDayPayload(runningPayload).Close < candleToCheck.Close Then
                                    If currentDayPayload(runningPayload).Close < leadingSpanAPayload(runningPayload) AndAlso
                                        currentDayPayload(runningPayload).Close < leadingSpanBPayload(runningPayload) Then
                                        Dim row As DataRow = ret.NewRow
                                        row("Date") = inputPayload(runningPayload).PayloadDate.ToString("dd-MMM-yyyy")
