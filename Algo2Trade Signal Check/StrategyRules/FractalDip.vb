                ...
                For Each stock In stockList
                    ...
                    If stockPayload IsNot Nothing AndAlso stockPayload.Count > 0 Then
                        ...
                        If currentDayPayload IsNot Nothing AndAlso currentDayPayload.Count > 0 Then
                            ...
                            Dim lastFractalLow As Decimal = Decimal.MinValue
                            For Each runningPayload In currentDayPayload.Keys
                                _canceller.Token.ThrowIfCancellationRequested()
                                If lastFractalLow <> Decimal.MinValue AndAlso fractalHighPayload(runningPayload) < lastFractalLow Then
                                    ...
                                    ret.Rows.Add(row)
                                    lastFractalLow = Decimal.MinValue
                                ElseIf lastFractalHigh <> Decimal.MinValue AndAlso fractalLowPayload(runningPayload) > lastFractalHigh Then
                                    Dim row As DataRow = ret.NewRow
                                    row("Date") = inputPayload(runningPayload).PayloadDate
                                    row("Trading Symbol") = inputPayload(runningPayload).TradingSymbol
                                    row("Direction") = "Sell"
                                    ...
                                    ret.Rows.Add(row)
                                    lastFractalHigh = Decimal.MinValue
                                End If
                                If lastFractalHigh = Decimal.MinValue Then
                                    lastFractalHigh = fractalHighPayload(runningPayload)
                                Else
                                    If fractalHighPayload(runningPayload) <> fractalHighPayload(currentDayPayload(runningPayload).PreviousCandlePayload.PayloadDate) Then
                                        lastFractalHigh = fractalHighPayload(currentDayPayload(runningPayload).PreviousCandlePayload.PayloadDate)
                                    End If
                                End If
                                If lastFractalLow = Decimal.MinValue Then
                                    lastFractalLow = fractalLowPayload(runningPayload)
                                Else
                                    If fractalLowPayload(runningPayload) <> fractalLowPayload(currentDayPayload(runningPayload).PreviousCandlePayload.PayloadDate) Then
                                        lastFractalLow = fractalLowPayload(currentDayPayload(runningPayload).PreviousCandlePayload.PayloadDate)
                                    End If
                                End If
                            Next
                        End If
