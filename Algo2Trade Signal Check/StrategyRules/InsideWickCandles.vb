...
Public Class InsideWickCandles
    Inherits Rule
                                ...
                                If currentDayPayload(runningPayload).PreviousCandlePayload IsNot Nothing Then
                                    If currentDayPayload(runningPayload).CandleColor = Color.Red Then
                                        If currentDayPayload(runningPayload).PreviousCandlePayload.CandleColor = Color.Red Then
                                            If currentDayPayload(runningPayload).Close >= currentDayPayload(runningPayload).PreviousCandlePayload.Open AndAlso
                                                currentDayPayload(runningPayload).Open <= currentDayPayload(runningPayload).PreviousCandlePayload.High Then
                                                Dim row As DataRow = ret.NewRow
                                                row("Date") = runningPayload
                                                row("Instrument") = currentDayPayload(runningPayload).TradingSymbol
                                                row("Wick Side") = "Top"
                                                ret.Rows.Add(row)
                                            ElseIf currentDayPayload(runningPayload).Open <= currentDayPayload(runningPayload).PreviousCandlePayload.Close AndAlso
                                                currentDayPayload(runningPayload).Close >= currentDayPayload(runningPayload).PreviousCandlePayload.Low Then
                                                Dim row As DataRow = ret.NewRow
                                                row("Date") = runningPayload
                                                row("Instrument") = currentDayPayload(runningPayload).TradingSymbol
                                                row("Wick Side") = "Bottom"
                                                ret.Rows.Add(row)
                                            End If
                                        ElseIf currentDayPayload(runningPayload).PreviousCandlePayload.CandleColor = Color.Green Then
                                            If currentDayPayload(runningPayload).Close >= currentDayPayload(runningPayload).PreviousCandlePayload.Close AndAlso
                                                currentDayPayload(runningPayload).Open <= currentDayPayload(runningPayload).PreviousCandlePayload.High Then
                                                Dim row As DataRow = ret.NewRow
                                                row("Date") = runningPayload
                                                row("Instrument") = currentDayPayload(runningPayload).TradingSymbol
                                                row("Wick Side") = "Top"
                                                ret.Rows.Add(row)
                                            ElseIf currentDayPayload(runningPayload).Open <= currentDayPayload(runningPayload).PreviousCandlePayload.Open AndAlso
                                                currentDayPayload(runningPayload).Close >= currentDayPayload(runningPayload).PreviousCandlePayload.Low Then
                                                Dim row As DataRow = ret.NewRow
                                                row("Date") = runningPayload
                                                row("Instrument") = currentDayPayload(runningPayload).TradingSymbol
                                            ...
                                            End If
                                        End If
                                    ElseIf currentDayPayload(runningPayload).CandleColor = Color.Green Then
                                        If currentDayPayload(runningPayload).PreviousCandlePayload.CandleColor = Color.Red Then
                                            If currentDayPayload(runningPayload).Open >= currentDayPayload(runningPayload).PreviousCandlePayload.Open AndAlso
                                                ...
                                                row("Wick Side") = "Top"
                                                ret.Rows.Add(row)
                                            ElseIf currentDayPayload(runningPayload).Close <= currentDayPayload(runningPayload).PreviousCandlePayload.Close AndAlso
                                                currentDayPayload(runningPayload).Open >= currentDayPayload(runningPayload).PreviousCandlePayload.Low Then
                                                Dim row As DataRow = ret.NewRow
                                                row("Date") = runningPayload
                                                row("Instrument") = currentDayPayload(runningPayload).TradingSymbol
                                                row("Wick Side") = "Bottom"
                                            ...
                                            End If
                                        ElseIf currentDayPayload(runningPayload).PreviousCandlePayload.CandleColor = Color.Green Then
                                            If currentDayPayload(runningPayload).Open >= currentDayPayload(runningPayload).PreviousCandlePayload.Close AndAlso
                                                currentDayPayload(runningPayload).Close <= currentDayPayload(runningPayload).PreviousCandlePayload.High Then
                                                Dim row As DataRow = ret.NewRow
                                                row("Date") = runningPayload
                                                row("Instrument") = currentDayPayload(runningPayload).TradingSymbol
                                                row("Wick Side") = "Top"
                                                ret.Rows.Add(row)
                                            ElseIf currentDayPayload(runningPayload).Close <= currentDayPayload(runningPayload).PreviousCandlePayload.Open AndAlso
                                                currentDayPayload(runningPayload).Open >= currentDayPayload(runningPayload).PreviousCandlePayload.Low Then
                                                Dim row As DataRow = ret.NewRow
                                                row("Date") = runningPayload
                                                row("Instrument") = currentDayPayload(runningPayload).TradingSymbol
