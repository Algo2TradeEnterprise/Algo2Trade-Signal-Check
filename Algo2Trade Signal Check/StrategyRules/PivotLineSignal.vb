                            ...
                            If previousPivot.Pivot < prePreviousPivot.Pivot Then
                                If currentDayFirstCandle.PreviousCandlePayload.Close > previousPivot.Pivot Then
                                    ...
                                    For Each runningPayload In previousDayPayload
                                        _canceller.Token.ThrowIfCancellationRequested()
                                        If runningPayload.Value.Close < previousPivot.Support1 Then
                                            valid = False
                                            Exit For
                                        End If
                                    Next
                                    If valid Then
                                        ...
                                        row("Open %") = Math.Round(((currentDayFirstCandle.Open / currentDayFirstCandle.PreviousCandlePayload.Close) - 1) * 100, 4)
                                        row("Low %") = Math.Round(((currentDayFirstCandle.Low / currentDayFirstCandle.PreviousCandlePayload.Close) - 1) * 100, 4)
                                        row("High %") = Math.Round(((currentDayFirstCandle.High / currentDayFirstCandle.PreviousCandlePayload.Close) - 1) * 100, 4)
                                        row("Close %") = Math.Round(((currentDayFirstCandle.Close / currentDayFirstCandle.PreviousCandlePayload.Close) - 1) * 100, 4)
